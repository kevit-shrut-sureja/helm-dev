...
// SIGTERM is a request. A hung event loop, a connection nothing is draining, or
// simply no shutdown hook means it can go unanswered forever — with no follow-up
// the service just sat in "stopping" indefinitely. SIGKILL cannot be ignored.
...
export class ServiceRunner extends EventEmitter {
  ...
  #ingest(service, stream, line) {
    ...
    // Nx prints this on every watch-mode restart too, which is normal for an app
    // that is already up — but while still "starting", an app's exit is exactly
    // as much a failure as a job's; nothing distinguishes "crashed" from "about
    // to restart" until it has booted once.
    if (exited !== null && active && active.status !== 'stopping' && (active.kind === 'job' || active.status === 'starting')) {
  ...
  start(name) {
    ...
    const entry = {
      ...
      readyTimer: null,
      killTimer: null,
      // Set once, the moment stop() decides to signal the process — not inferred
      // from entry.status, because #forceKill has to advance that status to
      // 'stopped' before the OS confirms the kill, and the exit handler runs
      // after that: checking entry.status there would find 'stopped', not
      // 'stopping', and misread its own forced kill as an unprompted crash.
      stoppedByUs: false,
  ...
  stop(name) {
    ...
    this.#releaseSlot(name);
    entry.killTimer = setTimeout(() => this.#forceKill(name), KILL_GRACE_MS);
    return true;
  }

  /**
   * Escalates to SIGKILL when a service is still "stopping" after its grace
   * period — SIGTERM was ignored, or something is keeping the event loop alive.
   * Descendants are re-scanned rather than reusing the original list, in case
   * the hung process spawned something new in the meantime.
   * @param name - the project name
   */
  #forceKill(name) {
    const entry = this.#processes.get(name);
    if (!entry || entry.status !== 'stopping' || entry.child.pid === undefined) return;
    const pid = entry.child.pid;
    for (const child of descendantsOf(pid)) {
      try {
    ...
    try {
      ...
      try {
        ...
        // The pid is already gone by the time this fires.
      }
    }
    // SIGKILL cannot be caught, so the 'exit' event is coming — but it is not
    // instant, and the UI should stop calling this "stopping" once nothing
    // short of a dead process remains possible.
    this.#setStatus(name, 'stopped', { forced: true });
  }

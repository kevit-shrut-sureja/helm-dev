...
// Which import prefix marks a shared library is a property of the repo, not of
// helm-dev, so it is read from the workspace's own TypeScript path aliases.
const DEFAULT_SCOPES = ['@chatomate'];
...
/**
 * Reads the workspace's TypeScript path aliases to learn which import prefixes
 * mean "a library in this repo".
 * @param repoRoot - absolute path to the monorepo root
 * @returns the scopes found, e.g. ['@chatomate']
 */
async function workspaceScopes(repoRoot) {
  for (const file of ['tsconfig.base.json', 'tsconfig.json']) {
    try {
      const raw = await readFile(join(repoRoot, file), 'utf8');
      // These files routinely carry comments, which JSON.parse will not accept.
      const paths = JSON.parse(raw.replace(/\/\*[\s\S]*?\*\/|(^|\s)\/\/.*$/gm, '$1')).compilerOptions?.paths;
      const scopes = [...new Set(Object.keys(paths ?? {}).map((alias) => alias.split('/')[0]))].filter((scope) =>
        scope.startsWith('@'),
      );
      if (scopes.length > 0) return scopes;
    } catch {
      // Try the next candidate, then fall back.
    }
  }
...
export async function resolveDependencies(repoRoot, sourceRoot) {
  ...
  const scopes = await workspaceScopes(repoRoot);
  const importRe = new RegExp(`(?:${scopes.map((scope) => scope.replace('@', '@')).join('|')})\\/([a-z0-9-]+(?:\\/[a-z0-9-]+)?)`, 'g');
...
export class SourceWatcher extends EventEmitter {
  ...
  async track(name, sourceRoot) {
    this.untrack(name);
    // One watch for the service's own source. Its libraries are covered by a
    // single shared watch over libs/ — a service here imports ~66 of them, and
    // one inotify instance each would exhaust the default limit on many machines.
    const own = join(this.#repoRoot, sourceRoot);
    const deps = await resolveDependencies(this.#repoRoot, sourceRoot);
    this.#watched.set(name, { changed: new Set(), roots: [own], deps, timer: null });
    this.#share(own, name);
    this.#watchLibraries();
  }

  /**
   * Opens one recursive watch over the whole libs/ tree, shared by every service,
   * and routes each change only to the services that import that library.
   */
  #watchLibraries() {
    if (this.#libsWatch !== null) return;
    const libs = join(this.#repoRoot, 'libs');
    if (!existsSync(libs)) return;
    try {
      this.#libsWatch = watch(libs, { recursive: true }, (_event, file) => {
        if (typeof file !== 'string' || !/\.(ts|html|scss|json)$/.test(file)) return;
        const absolute = join(libs, file);
        const changed = relative(this.#repoRoot, absolute);
        for (const [name, entry] of this.#watched) {
          if (entry.deps?.some((dep) => absolute.startsWith(`${dep}/`))) this.#record(name, changed);
        }
      });
    } catch {
      // Without it, library edits simply do not mark a service stale.
    }
  }
  ...
  untrack(name) {
    ...
    if (this.#watched.size === 0 && this.#libsWatch !== null) {
      this.#libsWatch.close();
      this.#libsWatch = null;
    }
  }
   ...
   */
  watchCount() {
    return this.#shared.size + (this.#libsWatch === null ? 0 : 1);
  }
}

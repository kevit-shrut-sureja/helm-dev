...
export class LogTailer extends EventEmitter {
  ...
  #pending = new Map();
  #timer = null;
  // fs.watch and the poll interval both call #sweep(), and #readNew reads the
  // offset before its await, so two concurrent calls for the same file can both
  // see the same stale offset and re-deliver the same bytes. A busy file is
  // skipped rather than queued: the poll or the next write's watch event will
  // pick up whatever arrived in the meantime, at most 400ms later.
  #busy = new Set();
  ...
  async #readNew(path) {
    ...
    if (this.#busy.has(service)) return;
    this.#busy.add(service);
    try {
